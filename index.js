...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// because "nestedFunction" is the child of "myFunction" so it inherits the "Traits" or code from the parent. The same will not be true the other way around.
...
// myNum = 4. 1+2+3+4=10 expected output should be 10.

function summation(myNum) {
let finalNum = 0
  for(let i = 0; i < myNum; i++){
    finalNum = i + 1 + finalNum
  }
  return finalNum
}
...
console.log(animalNames(zooAnimals))
  ...
  */
  ...
  function lowerCaseNames(data){
    const nameInLowerCaseArray = data.map(function(item){
      return item.animal_name.toLowerCase()
    })
    return nameInLowerCaseArray
  }
  ...
  */
  ...
  function lowPopulationAnimals(data){
    const lowPopAnimalArray = data.filter(function(item){
...
console.log(lowPopulationAnimals(zooAnimals))
  ...
  */
  ...
  function USApop(data){
    const totalZooPopulation = data.reduce(function(accumulator, item){
